        ...
        <div v-if="detailVisible" class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="bg-white w-full max-w-md px-4 h-1/4 md:h-auto">
                    ...
                    <img class="max-h-[40rem]" :src='`http://34.125.67.227/images/uploads/${selectedSlip.slip_img}`' alt="">
                    <div class="h-3/12">
...
export default {
    ...
    methods: {
        ...
        approve(slip) {
            console.log(slip)
            axios.put('http://34.125.67.227/api/approve', slip).then(res => {
                console.log(res)
                this.$emit('slip');
            }).catch(err => {
                console.log(err)
        ...
        decline(slip) {
            console.log(slip)
            axios.put('http://34.125.67.227/api/decline', slip).then(res => {
                console.log(res)
                this.$emit('slip');
            }).catch(err => {
                console.log(err)
    ...
    created() {
        axios.get("http://34.125.67.227/api/allSlip")
            .then(res => {
