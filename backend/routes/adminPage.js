...
router.get("/formBook", isLoggedIn, async function (req, res, next) {
...
router.get("/editBook", isLoggedIn, async function (req, res, next) {
    if (req.user.type == "admin") {
        try {
...
router.get('/allSlip/', isLoggedIn, async (req, res, next) => {
...
router.get('/getAllCustomer/', isLoggedIn, async (req, res, next) => {
    if (req.user.type == "admin") {
        try {
            // select * from payment join cust_order using(order_id) join order_line using(order_id)
        ...
        res.status(409).send("You are not admin")
    }
...
router.put('/decline/', isLoggedIn, async (req, res, next) => {
    if (req.user.type == "admin") {
